...
const showBookings=(data)=>{
    ...
    arr.forEach(booking => {
        ...
        li.classList.add("w-full","max-w-[1000px]","bg-[#fcf2df]","h-auto","text-[15px]","rounded-[10px]","box-border","p-[13px]","flex","justify-between","items-center");
    ...
    if(!upcoming.hasChildNodes())
        upcoming.appendChild(emptyUpcoming);
    if(!previous.hasChildNodes())
        previous.appendChild(emptyPrevious);
}